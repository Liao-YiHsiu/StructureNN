...
#ifndef _NNET_MY_AFFINE_COMPONENT_H_
#define _NNET_MY_AFFINE_COMPONENT_H_

#include "my-nnet/nnet-my-component.h"

class myAffine : public MyComponent {
   public:
      myAffine(int32 dim_in, int32 dim_out) 
         : MyComponent(dim_in, dim_out), 
   ...
   { }
      ...
      MyComponent* Copy() const { return new myAffine(*this); }

      virtual MyType GetType() const { return mAffine; }
   ...
   public:
      ...
      void Update(const CuMatrixBase<BaseFloat> &input, const CuMatrixBase<BaseFloat> &diff) {
         // initialize
         if(linearity_acc_.NumRows() == 0){
            linearity_acc_.Resize(linearity_corr_.NumRows(), linearity_corr_.NumCols());
            bias_acc_.Resize(bias_corr_.Dim());
            num_rows = 0;
         }

         linearity_acc_.AddMatMat(1.0, diff, kTrans, input, kNoTrans, 1.0);
         bias_acc_.AddRowSumMat(1.0, diff, 1.0);
      ...
      void Update(){
         ...
         const BaseFloat l1 = opts_.l1_penalty;
         // we will also need the number of frames in the mini-batch
         const int32 num_frames = num_rows;
         // compute gradient (incl. momentum)
         linearity_corr_.AddMat(mmt, linearity_acc_);
         bias_corr_.AddVec(mmt, bias_acc_);
         // l2 regularization
         if (l2 != 0.0) {
            linearity_.AddMat(-lr*l2*num_frames, linearity_);
         }
         ...
         linearity_acc_.SetZero();
         bias_acc_.SetZero();
         num_rows = 0;
      }
   private:
      CuMatrix<BaseFloat> linearity_acc_;
